...
# MERGE VCF
# this script subsets the first 2000000 basepair positions (not variants!)
# from chromosomes 1 through 5, and stores them as separate, small (subset)  vcf files
...
bcftools concat $example_vcf_dir"chr1.small.vcf.gz" $example_vcf_dir"chr2.small.vcf.gz" $example_vcf_dir"chr3.small.vcf.gz" $example_vcf_dir"chr4.small.vcf.gz" $example_vcf_dir"chr5.small.vcf.gz" > $merged_vcf_file
...
cat $thousand_genome_map_dir"plink.chr5.GRCh38.map" >> $merged_map_file
