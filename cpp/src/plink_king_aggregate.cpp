...
map<string, map<string, float>> main_plink_aggregate(string plink_file, char delim=' '){
    ...
    cout << "READING PLINK FILE..." << plink_file << endl;
    ...
    while (getline(plink_file_stream, line)) {
        if (header == "") {
            ...
            string sampleID_1 = vec_line[0];
            string sampleID_2 = vec_line[1];
            float kinship_value = stof(vec_line[5]);

            // fill map with sample 1 as key
            if (plink_map.find(sampleID_1) != plink_map.end()) {
                plink_map[sampleID_1][sampleID_2] = kinship_value;
            } else {
                sampleID1_dist[sampleID_2] = kinship_value;
                plink_map[sampleID_1] = sampleID1_dist;
            }
            // fill map with sample 2 as key
            if (plink_map.find(sampleID_2) != plink_map.end()) {
                plink_map[sampleID_2][sampleID_1] = kinship_value;
            } else {
                sampleID2_dist[sampleID_1] = kinship_value;
                plink_map[sampleID_2] = sampleID2_dist;
            }
        }
    }
    cout << "DONE READING PLINK FILE" << endl;
    return plink_map;
}
