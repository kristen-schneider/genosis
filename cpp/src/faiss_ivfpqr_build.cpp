...
int main(int argc, char* argv[]){
	...
	faiss::IndexIVFPQR faiss_ivfpqr_index;
...
faiss::IndexIVFPQR build_write_ivfpqr_index(string database_IDs, string database_encodings, const char* index_out_file){
	...
	// make faiss index
	faiss::IndexIVFPQR faiss_ivfpqr_index = build_ivfpqr_index(vector_size, database_IDs, ID_encodings_map);
	cout << "writing index to " << index_out_file << endl;
	faiss::write_index(&faiss_ivfpqr_index, index_out_file);
	return faiss_ivfpqr_index;
}


faiss::IndexIVFPQR build_ivfpqr_index(int vector_size, string database_IDs, map<string, float*> ID_encodings_map){
//const FaissIndex* build_ivfpqr_index(int vector_size, string database_IDs, map<string, float*> ID_encodings_map, string index_file){
	cout << "building faiss ivfpqr index." << endl;
	
	faiss::IndexIVFPQR index(vector_size, 40);
	index.ivfpqr.efSearch = 40;
