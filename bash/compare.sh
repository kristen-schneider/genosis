...
queries_file="/home/sdp/pmed-local/data/AFR/AFR_sample_IDs.txt"
...
$bin_dir"plink-genome" $out_dir"plink.1kg-chr8.genome.genome" $k $out_dir"topk_genome.AFR" $queries_file
echo "plink --genome runtime: $pg_runtime"

# plink --make-king-table
pk_start=`date +%s.%N`
plink2 --vcf $vcf --make-king-table --out $out_dir"plink.1kg-chr8.king"
$bin_dir"plink-king" $out_dir"plink.1kg-chr8.king.kin0" $k $out_dir"topk_king.AFR" $queries_file
pk_end=`date +%s.%N`
