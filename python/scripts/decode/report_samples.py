...
def write_combined_haps(sample_ID, hap0_results, hap1_results, out_dir):
    ...
    for match_id in hap0_results[sample_ID]:
        pop_file.write(' ' + match_id + ': ' + str(hap0_results[sample_ID][match_id][0]) + '\n')
        ibd_file.write(' ' + match_id + ': ' + str(hap0_results[sample_ID][match_id][0]) + '\n')

    pop_file.write(sample_ID + '_1\n')
    ibd_file.write(sample_ID + '_1\n')
    for match_id in hap1_results[sample_ID]:
        pop_file.write(' ' + match_id + ': ' + str(hap1_results[sample_ID][match_id][0]) + '\n')
        ibd_file.write(' ' + match_id + ': ' + str(hap1_results[sample_ID][match_id][1]) + '\n')
