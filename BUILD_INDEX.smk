...
# 2.0 list input VCF segments
rule list_VCF_segments:
	input:
		slice_log=f"{config.log_dir}slice.log"
	output:
		vcf_segments_txt=f"{config.vcf_segments_dir}vcf_segments.txt"
	message:
		"listing all input VCF segments"
	conda:
		f"{config.conda_pmed}"
	shell:
		"ls {config.vcf_segments_dir}*.vcf.gz > {config.vcf_segments_dir}vcf_segments.txt"
